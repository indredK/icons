...
import { C_oldUrl, D_oldUrl, S_oldUrl, templeName } from '../../constant';
import { getString_web, getSvgList } from '../../xzUtils/utils';
import AllIcons, { extractIdFromSvgString } from './AllIcons';
import { useSdk } from './rules/hook';
import { Crule, Srule, isDrule } from './rules/rule';
interface DataType {
  key: number;
  diff: string;
  num1: string;
    ...
    const init = async () => {
      const temple = {
        ...
        [templeName.channel]: {
          ...
          label: '通道类图标',
          rule: (it: string) =>
            Crule(extractIdFromSvgString(it), MODEL, MetricList),
          oldValue: [],
          value: [],
        },
        // [templeName.location]: {
      ...
      for (const item of Object.keys(temple)) {
        ...
        const oldvalue = await localforage.getItem<string>(
          `svgList-${item}-oldUrl`,
        );
  ...
  const [resList, Tdata] = useMemo(() => {
    ...
    const Tdata: DataType[] = Object.keys(data).map((item, index) => {
      return {
        ...
        num2: `总共${data[item].value.length}个，新增${
          data[item].value.length - data[item].oldValue.length
        }个`,
      };
    });
    ...
    <div>
      <Row>
        <Col span={12} style={{ padding: 20 }}>
              ...
              return (
                <Form.Item
                  ...
                  label={
                    <div>
                      {data[item].label}{' '}
                      {data[item].url !== data[item].oldUrl && `(New)`}{' '}
                    </div>
                  }
                  rules={[
                    { required: false },
          ...
          {Object.keys(data).map((item, index) => {
            return (
              <Col span={24}>
                {data[item].label} ： 新增
                {data[item].value.length - data[item].oldValue.length}个；
        ...
        <Col span={12} style={{ padding: 20 }}>
          ...
          <h3>可以在这里验证更新命名以后图标的正确性：</h3>
          <h4>1、更新链接以后可以看到最新的效果</h4>
          <h4>
            2、如果需要发布，就必须拉代码下来，然后更新链接运行脚本提交代码，发布交给CI
          </h4>
          <h4>3、非得手动发布也可以，那就看项目发布流程</h4>
        </Col>
      </Row>
    </div>
  );
